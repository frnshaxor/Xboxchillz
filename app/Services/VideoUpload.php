...
class VideoUpload
{
    public const CHUNK_SIZE = 5 * 1024 * 1024; // 5MB per chunk
    ...
    private function processOne(string $title, int $categoryId, array $f, int $limitMb): bool
    {
        ...
        if (!$isMp4 && $mime === 'application/octet-stream') {
            ...
            if ($handle) {
                ...
                fclose($handle);
                $isMp4 = str_contains($header, 'ftyp');
            }
        }
        if (!$isMp4) {
            return false;
        ...
        if ($probe) {
            $duration = (int) round((float) trim($probe));
        }
    ...
    public function assembleAndProcess(string $uploadId, int $categoryId, string $title): array
    {
        ...
        if (!$out) {
            ...
            return ['error' => 'Gagal menulis file output'];
        }
        foreach ($chunks as $c) {
