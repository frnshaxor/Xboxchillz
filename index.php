...
if ($page === 'save-settings' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach (['site_name', 'site_description', 'accent'] as $k) {
...
if ($page === 'upload' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title && $f && $f['error'] === UPLOAD_ERR_OK) {
        $ext = strtolower(pathinfo($f['name'], PATHINFO_EXTENSION));
        if ($ext !== 'mp4') { go('?page=admin&err=ext'); }
        if ($f['size'] > $limit_mb * 1024 * 1024) { go('?page=admin&err=size'); }
        $fi = new finfo(FILEINFO_MIME_TYPE);
        $mime = $fi->file($f['tmp_name']) ?: '';
        // Secondary check: verify MP4 magic bytes (ftyp box) when finfo is uncertain
        ...
        if (!$isMp4) { go('?page=admin&err=mime'); }

        $slug = preg_replace('/[^a-z0-9]+/i', '-', strtolower($title)) . '-' . bin2hex(random_bytes(3));
        $dir = MEDIA_ROOT . '/' . $slug;
        mkdir($dir, 0750, true);
        ...
        // Queue all CPU-heavy work outside the upload request. The worker creates the poster,
        // renditions and master playlist, then updates the already-inserted DB row.
        ...
        $s->bind_param('ssissiis', $title, $slug, $cat, $poster, $src, $duration, $size, $status);
        $s->execute();
        log_activity($db, (int)$_SESSION['admin_id'], 'video_upload', $slug);

    $worker = '/usr/local/sbin/arsip-hls-worker';
    if (is_executable($worker)) {
        shell_exec('nohup ' . escapeshellarg($worker) . ' ' . escapeshellarg($slug) . ' > /dev/null 2>&1 < /dev/null &');
    }
    }
...
if ($page === 'verify-token' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $s = $db->prepare("SELECT id FROM access_tokens WHERE token=? AND status='active'");
...
if ($page === 'midtrans-notify' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($newStatus === 'settlement' && !$tokenId) {
        ...
        $create = $db->prepare('INSERT INTO access_tokens(token,label,contact_type,contact_value,status) VALUES(?,?,?,?,?)');
        $create->bind_param('sssss', $token, $label, $contactType, $contact, $active); $create->execute(); $tokenId = $db->insert_id;
...
if ($page === 'media' || $page === 'poster') {
    $relative = rawurldecode((string)($_GET['path'] ?? ''));
    $relative = preg_replace('#^/?(?:protected-media/|media/)#', '', $relative);
    $allowed = '#^[a-z0-9][a-z0-9-]*/(?:poster\.jpg|source\.mp4|master\.m3u8|(?:360p|720p)\.m3u8|(?:360p|720p)_\d{3}\.ts)$#i';
    ...
    if ($page === 'media' && (!$isPoster || !has_access()) && !admin()) { http_response_code(403); exit('Akses token diperlukan.'); }
...
if ($page === 'token-create' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $status = 'active';
    $q = $db->prepare('INSERT INTO access_tokens(token,label,contact_type,contact_value,status,created_by) VALUES(?,?,?,?,?,?)');
    $q->bind_param('sssssi', $token, $label, $contactType, $contactValue, $status, $adminId); $q->execute();
...
$accent = setting($db, 'accent', '#d96b45');
...
<style>:root{--accent-user:<?= e($accent) ?>}.material-symbols-rounded{font-variation-settings:'FILL' 0,'wght' 400,'GRAD' 0,'opsz' 24;vertical-align:middle}</style>
...
<script src="https://cdn.jsdelivr.net/npm/plyr@3.7.8/dist/plyr.min.js" defer></script>
      ...
      <div class="grid2">
        <div class="panel">
          ...
          <form id="upload-form" method="post" enctype="multipart/form-data" action="?page=upload" data-testid="upload-form">
            <input type="hidden" name="csrf" value="<?= csrf() ?>">
            <label>Judul<input name="title" required data-testid="upload-title"></label>
            ...
            <label>File MP4 (maks <?= $upload_mb ?> MB)<input type="file" name="video" accept="video/mp4" required data-testid="upload-file"></label>
        ...
        <div class="panel">
          ...
          <form method="post" action="?page=save-settings" data-testid="identity-form">
            ...
            <label>Warna aksen<input name="accent" type="color" value="<?= e($accent) ?>" data-testid="setting-accent" style="height:48px;padding:6px"></label>
        ...
        <div class="table">
          ...
          <?php while ($v = $vids->fetch_assoc()): ?>
            <div class="tr">
              ...
              <span><span class="status <?= e($v['status']) ?>"><?= e($v['status']) ?></span></span>
              <span>
            ...
            <?php while ($token = $fallbackTokens->fetch_assoc()): ?>
              <article class="token-fallback-row">
                ...
                <div class="token-fallback-actions">
                  <span class="status <?= $token['status'] === 'active' ? 'ready' : 'processing' ?>"><?= e($token['status']) ?></span>
...
<main class="watch"><div class="wrap">
  ...
  <h1 style="margin-top:10px"><?= e($v['title']) ?></h1>
  ...
  <?php else: ?>
  <!-- ── LOCKED: blurred poster + token modal trigger ── -->
  <div class="player-locked" id="player-locked">
    <div class="locked-poster">
      <?php if (is_file(APP_ROOT . '/' . ltrim($v['poster'], '/'))): ?>
        <img src="<?= e(poster_url($v['poster'])) ?>" alt="<?= e($v['title']) ?>">
      <?php endif; ?>
      <div class="locked-overlay">
        <span class="material-symbols-rounded locked-icon">lock</span>
        <p class="locked-msg">Masukkan token akses untuk menonton video ini</p>
  ...
  <div class="video-meta">
    <div><span class="material-symbols-rounded">schedule</span><?= gmdate('i:s', (int)$v['duration_sec']) ?></div>
    <div><span class="material-symbols-rounded">lock</span>Akses terkunci</div>
  </div>
  ...
  <div class="token-modal-overlay" id="token-modal" role="dialog" aria-modal="true" aria-labelledby="token-modal-title">
    <div class="token-modal-card">
      ...
      <p class="token-modal-hint">Belum punya token? Hubungi admin untuk mendapatkan akses.</p>
      <?php $contactUrl = setting($db, 'contact_url', ''); $contactLabel = setting($db, 'contact_label', 'Hubungi admin'); if ($contactUrl): ?>
      <a class="token-contact" href="<?= e($contactUrl) ?>" target="_blank" rel="noopener noreferrer"><span class="material-symbols-rounded">send</span> <?= e($contactLabel) ?></a>
      <?php endif; ?>
...
<main class="home"><div class="wrap">
  <section class="hero">
    <div>
      ...
      <h1>Layar untuk cerita yang<br>ingin tinggal lebih&nbsp;lama.</h1>
      <p style="margin-top:22px"><?= e($desc) ?></p>
    </div>
    <div class="index"><big>01</big>Watch · Listen · Return</div>
  </section>
